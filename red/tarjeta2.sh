...
tput sgr0
echo -e "\n ${bold}`tput setab 4` $netcard3${normal}"
echo -e "\n ${bold}DIRECCIÓN IPv4:${normal} `tput setaf 2`$addrsnetcard3${normal}"
echo -e " ${bold}PUERTA DE ENLACE:${normal} `tput setaf 2`$gatewaydinamic3${normal}"
if [ $dnsnetcard3 ]; then
echo -e " ${bold}SERVIDORES DNS:${normal} `tput setaf 2`$dnsnetcard3${normal}\n"
fi
...
read -p "Elige una opción: " choice8
    ...
    1)
		...
		read -p "¿Quieres configurar una direccion ip estática`tput setaf 2`(e)${normal} o dinámica`tput setaf 3`(d)${normal}?: " choice9
			...
			e)
				...
				numerodelineas=$(cat /etc/netplan/01-installer-config.yaml | wc -l)
				...
				if [ $numerodelineas -lt 5 ]; then
					...
					#primero borramos el contenido del fichero dhcp.yaml
						...
						echo "  ethernets:" >> /etc/netplan/01-installer-config.yaml
						echo "    $netcard3:" >> /etc/netplan/01-installer-config.yaml
						echo "      addresses: [$ipv4]" >> /etc/netplan/01-installer-config.yaml
						echo "      gateway4: $gt" >> /etc/netplan/01-installer-config.yaml
						echo "      nameservers:" >> /etc/netplan/01-installer-config.yaml
						echo "         addresses: [$dns]" >> /etc/netplan/01-installer-config.yaml
						netplan apply
						echo -e "\n"
						echo -e "`tput setaf 2`Ha añadido la dirección $ipv4 a la tarjeta de red $netcard3${normal}"
						echo -e "\n"
					...
					echo -e ""
					read -p "Ya existe una dirección ip configurada para $netcard3, ¿quiere modificar esta dirección? (s/n): " eleccion
				fi
				...
				if [ $eleccion == "s" ]; then
						...
						sed -i "s:$ipv4exist:$ipv4noexist:g" /etc/netplan/01-installer-config.yaml
			...
			d)
				...
				case $choice11 in
					s)
						...
						echo "network:" > /etc/netplan/01-installer-config-dhcp.yaml
							echo "  version: 2" >> /etc/netplan/01-installer-config-dhcp.yaml
							echo "  ethernets:" >> /etc/netplan/01-installer-config-dhcp.yaml
						echo "    $netcard3:" >> /etc/netplan/01-installer-config-dhcp.yaml
						echo "       dhcp4: yes" >> /etc/netplan/01-installer-config-dhcp.yaml
							netplan apply
							echo -e "\nHa configurado la IP de manera dinámica para $netcard3\n"
	...
	;;
	2)
		if [ -n "${dnsnetcard3}" ]; then
		echo -e "\nTu dirección DNS es $dnsnetcard3\n"
		read -p "Escribe la nueva dirección DNS (sin corchetes): " dnsnew		
		sed -i "8 s/\[\([^]]*\)\]/\[$dnsnew]/g" /etc/netplan/01-installer-config.yaml
		echo "Ha cambiado la dirección $dnsnetcard3 por la dirección [$dnsnew]"
		else
	...
	3)
		echo -e "\n"
		read -p "¿Quieres activar(1) o desactivar(2) la tarjeta de red $netcard3?: " choice12
			case $choice12 in
				1)
					ip link set $netcard3 up  
					echo -e "\nLa tarjeta $netcard3 está activada\n"
					read -p "Pulse cualquier botón para continuar"
					. ./red/menured.sh
				;;
				2)
					ip link set $netcard3 down
					echo -e "\nLa tarjeta $netcard3 está desactivada\n"
					read -p "Pulse cualquier botón para continuar"
					. ./red/menured.sh
				;;
				esac
